...
export default function Post() {
  ...
  useEffect(() => {
    const fetchPostOwner = async () => {
      try {
        const response = await fetch(`http://localhost:5000/api/users/${post.userId}`, {
          headers: {
  ...
  const getProfileImageUrl = () => {
    ...
    return `http://localhost:5000${postOwner.profileImage}`;
  };

  if (!post) {
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-black">Post not found</div>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen w-full bg-white text-black">
      {/* Left Sidebar */}
      <div className="flex flex-col w-60 h-screen overflow-hidden border-r border-gray-200">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="flex items-center p-4 border-b border-gray-200">
          <button
            onClick={() => navigate(-1)}
            className="p-2 rounded-full hover:bg-[#6f85e5] mr-4"
          >
